...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['addpekerjaan'])) {
    ...
    if (isset($_FILES['fotopekerjaan']) && $_FILES['fotopekerjaan']['error'] === 0) {
        $fotopekerjaan = $_FILES['fotopekerjaan']['name'];
        $target_dir = '../assets/images/';
        $target_file = $target_dir . basename($fotopekerjaan);
        $imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION)); // Perbaikan: Variabel typo

        if ($_FILES["fotopekerjaan"]["size"] > 2000000) { // Perbaikan: Sintaks akses size array
        ...
        if (!isset($error_message)) {
            if (move_uploaded_file($_FILES['fotopekerjaan']['tmp_name'], $target_file)) {
                ...
                $query = "INSERT INTO pekerjaan (namapekerjaan, deskripsipekerjaan, fotopekerjaan, tahunmulai, tahunakhir) VALUES (?, ?, ?, ?, ?)"; // Perbaikan: Nama tabel dan urutan kolom
                $stmt = $conn->prepare($query);
                $stmt->bind_param("sssss", $namapekerjaan, $deskripsipekerjaan, $fotopekerjaan, $tahunmulai, $tahunakhir); // Perbaikan: Jumlah dan tipe binding parameter
                $stmt->execute();
                header("Location: crudpekerjaan.php"); // Perbaikan: Nama file redirect
                exit;
            } else {
        ...
        $query = "INSERT INTO pekerjaan (namapekerjaan, deskripsipekerjaan, tahunmulai, tahunakhir) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssss", $namapekerjaan, $deskripsipekerjaan, $tahunmulai, $tahunakhir); // Perbaikan: Jumlah dan tipe binding parameter
        $stmt->execute();
        header("Location: crudpekerjaan.php"); // Perbaikan: Nama file redirect
        exit;
    }
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="text-center mb-4">Tambah Pekerjaan</h2>
        <form method="POST" enctype="multipart/form-data" class="row g-3">
            <div class="col-md-6">
                <label for="namapekerjaan" class="form-label">Nama Pekerjaan</label>
                <input type="text" name="namapekerjaan" class="form-control" id="namapekerjaan" required>
            </div>
            <div class="col-md-6">
                <label for="deskripsipekerjaan" class="form-label">Deskripsi Pekerjaan</label>
                <input type="text" name="deskripsipekerjaan" class="form-control" id="deskripsipekerjaan" required>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-6">
                ...
                <input type="file" name="fotopekerjaan" class="form-control" id="fotopekerjaan" required>
            </div>
            <div class="col-12 text-center">
                <button type="submit" name="addpekerjaan" class="btn btn-success">Tambah Pekerjaan</button>
            </div>
        </form>

        <h3 class="mt-5 mb-3">Daftar Pekerjaan</h3>
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    while ($product = $result->fetch_assoc()) {
                        echo "<tr>
                            <td>" . htmlspecialchars($product['namapekerjaan']) . "</td>
                            <td>" . htmlspecialchars($product['deskripsipekerjaan']) . "</td>
                            <td><img src='../assets/images/" . htmlspecialchars($product['fotopekerjaan']) . "' alt='Foto Pekerjaan' width='100' class='img-thumbnail'></td>
                            <td>" . htmlspecialchars($product['tahunmulai']) . "</td>
                            <td>" . htmlspecialchars($product['tahunakhir']) . "</td>
                            <td>
                                <a href='editpekerjaan.php?id=" . htmlspecialchars($product['idpekerjaan']) . "' class='btn btn-primary btn-sm me-1'><i class='fas fa-edit'></i> Ubah</a>
                                <a href='hapuspekerjaan.php?id=" . htmlspecialchars($product['idpekerjaan']) . "' class='btn btn-danger btn-sm'><i class='fas fa-trash-alt'></i> Hapus</a>
                            </td>
                        </tr>";
                    }
                    ?>
